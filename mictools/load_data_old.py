...
def file_names(scanno, detector, path):
    files = glob(path+detector.upper()+f"/scan_{scanno:04d}_*.h5")
    return files

def get_scan_info(scanno, detector, path):
    data_dic = {}
...
def load_image_from_scan(imno, scanno, detector, path):
    files = file_names(scanno, detector, path)
...
def load_data_from_roi(scanno, detector, path, roi):
    ...
    flyscan HDF5 files. Returns an Nx4 array where the first column is 
    timestamps, and the following columns are intensity in ROI, COM y-position, and
    COM x-position.
    ...
    y_coords = np.arange(roi[0], roi[1])
    x_coords = np.arange(roi[2], roi[3])
    yy, xx = np.meshgrid(y_coords, x_coords, indexing='ij')
    
    for file in files:
        with File(file, "r") as f:
            dset = f["entry/data/data"]
            data_roi = dset[:, roi[0]:roi[1], roi[2]:roi[3]]
    ...
    com_y = np.concatenate(com_y_list, axis=0)
    com_x = np.concatenate(com_x_list, axis=0)
    times = np.concatenate(times_list, axis=0)
...
def load_interferometry_data(scanno, path, reduction=1):
